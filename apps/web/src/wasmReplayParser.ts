...
export async function extractReplayDirectItemPurchasesWithWasm(
  buffer: ArrayBuffer,
  profileJson = DEFAULT_DECODER_PROFILE_REGISTRY_JSON,
): Promise<ReplayDirectItemPurchasesResult> {
  return withReplayAndProfileBuffers(
    buffer,
    profileJson,
    (module, replayPointer, size, profilePointer, profileSize) => {
      const extractDirectItemPurchases = module.cwrap<
        (input: number, size: number, profiles: number, profileSize: number) => number
      >("lra_extract_replay_direct_item_purchases_buffer_with_profiles", "number", [
        "number",
        "number",
        "number",
        "number",
      ...
      return parseJsonResult<ReplayDirectItemPurchasesResult>(
        module,
        extractDirectItemPurchases(replayPointer, size, profilePointer, profileSize),
      );
    },
  );
}

export async function extractReplayItemSalesWithWasm(
  buffer: ArrayBuffer,
  profileJson = DEFAULT_DECODER_PROFILE_REGISTRY_JSON,
): Promise<ReplayItemSalesResult> {
  return withReplayAndProfileBuffers(buffer, profileJson, (module, replayPointer, size, profilePointer, profileSize) => {
    const extractItemSales = module.cwrap<
      (input: number, size: number, profiles: number, profileSize: number) => number
