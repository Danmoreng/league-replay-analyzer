...
- the default web landing page is a focused timeline product view: the scrubber and event list show only kills and objectives, while compact champion cards show portrait, player, role, scrub-time K/D/A, and replay-native lane/jungle CS for all 13 exact corpus builds; derived level is additionally shown where the 16.14 XP decoder is available. Ward, purchase, and sale streams remain Research & Debug concerns and do not appear on the product timeline. Cumulative total gold is not relabelled as current spendable gold, and partial item-operation streams are not presented as inventory slots or current inventory. Final inventories, fake health/resource bars, a minimap, decoder explanations, and ward-position research controls are absent. Current spendable gold and complete timeline inventory remain unavailable until replay-native decoders pass promotion. Static Data Dragon champion icons use the exact replay-build-to-Data-Dragon-version pin (`16.14.794.5912` to `16.14.1`), never a runtime `latest` lookup, and never create replay state
...
- keyframe outer framing and champion ownership are exact. The external profile now carries patch-specific keyframe packet/layout/cipher grammars for all nine corpus patch groups and 13 exact builds. Productive v4 lane/jungle-CS snapshots validate across all 57 replays: jungle CS 16,760/16,760 exact, lane CS 16,751/16,760 exact plus nine frozen same-keyframe ordering boundaries, with zero unaccepted mismatches. Six families have complete cipher permutations; 15.24, 16.1, and 16.5 use bounded partial-cipher domains and emit only invariant integer projections. Exact build `16.14.794.5912` additionally retains its separately validated XP, derived level, and cumulative total-gold fields; historical v4 snapshots leave those fields null. Current gold, health/resources, full inventory, movement, and the remaining inner state grammar remain unresolved
...
- external profile revision `2026-07-25-cross-patch-cs`, SHA-256 `a4ee89df1ff70e97fa97b9b64c632851584c110bc65af2301420a930e79307d3`, fingerprint `fnv1a64:5cf4895f9e6d3f4c`; its exact-build CS profiles validate 16,760/16,760 jungle-CS snapshots and 16,751/16,760 exact lane-CS snapshots plus nine accepted ordering boundaries across all 57 replays. For build `16.14.794.5912` it also validates 684/684 kills, 99/99 objectives, 1,477/1,477 ward placements, 484/484 ward removals, 1,300/1,300 final player-stat values, 193/193 purchase-linked resulting-item updates, 1,278/1,278 direct add-only item purchases, 116/116 item-sale operations, 3,200/3,200 keyframe levels, and 3,198/3,200 floored XP values; ward position has zero valid candidates and remains unavailable
...
1. preserve the promoted cross-patch lane/jungle-CS gate and extend historical XP/level only with a separate full replay-only validation; do not treat nullable historical v4 fields as decoded values
