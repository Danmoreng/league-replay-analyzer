...
- keyframe outer framing and champion ownership are exact. For exact build `16.14.794.5912`, every champion/keyframe has an exact 366-record `0x01EB` directory connected to one 1,479-byte `0x02EB` block by the exact bridge `0x0233 · (0x0306 0x0306)^k · 0x0452 · 0x007D?`, `k=0..4`; the `0x01EB` tokens remain a non-semantic physical directory. Separately, a D7-frozen profile-pinned 256-byte cipher→plain permutation (SHA-256 `c9be1f4971505dcc7c4366329366794108c1b031060039a2bcfd2d60134ed4be`) produces replay-native interleaved Float32LE keyframe snapshots: XP `[83,85,87,89]`, total gold `[115,117,119,121]`, and integral lane CS `[123,125,127,129]`; level is derived from XP and replay-embedded final cap. On 2,170 D7 and 1,030 H3 snapshots the productive fields pass their frozen corpus gates. This profile-backed surface is implemented through C++, Native, Wasm, and the product UI and fails closed outside the exact external profile. Neutral CS `[131,133,135,137]` remains a strong but unpromoted Float32 research candidate; current gold, health/resources, full inventory, movement, and the remaining inner state grammar remain unresolved
