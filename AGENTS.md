...
- the current web app loads a `.rofl` locally and calls the real Wasm decoders for replay-derived participant summaries, kills, elite objectives, ward lifecycle events, strict purchase-linked resulting-item updates, strict direct add-only item purchases, exact item-sale operations, and exact-build profile-backed keyframe XP/level/total-gold/lane-CS snapshots; not every decoded research stream is mounted in the product view
- the default web landing page is a focused timeline product view: the scrubber and event list show only kills and objectives, while compact champion cards show portrait, player, role, scrub-time K/D/A, replay-native XP/derived level, and lane/jungle CS for all 13 exact corpus builds. Ward, purchase, and sale streams remain Research & Debug concerns and do not appear on the product timeline. Cumulative total gold is not relabelled as current spendable gold, and partial item-operation streams are not presented as inventory slots or current inventory. Final inventories, fake health/resource bars, a minimap, decoder explanations, and ward-position research controls are absent. Current spendable gold and complete timeline inventory remain unavailable until replay-native decoders pass promotion. Static Data Dragon champion icons use the exact replay-build-to-Data-Dragon-version pin (`16.14.794.5912` to `16.14.1`), never a runtime `latest` lookup, and never create replay state
...
- keyframe outer framing and champion ownership are exact. The external profile carries patch-specific keyframe packet/layout/cipher grammars for all nine corpus patch groups and 13 exact builds. Productive v4 snapshots validate across all 57 replays: XP 16,746/16,760 exact plus 14 frozen ordering boundaries, derived level 16,759/16,760 exact plus one ordering boundary, jungle CS 16,760/16,760 exact, and lane CS 16,751/16,760 exact plus nine ordering boundaries, with zero unaccepted mismatches. Partial ciphers emit only invariant integer projections. Exact build `16.14.794.5912` additionally retains cumulative total gold; historical total gold remains null. A D7/H3 scan of the fully decoded 16.14 champion snapshot packet finds no promotable direct Float32 health/healthMax/resource/resourceMax lane. Current gold, health/resources, full inventory, movement, and the remaining inner state grammar remain unresolved
- exact-build current-gold research identifies `[107,109,111,113]` as a finite, integral cumulative spent-gold-like Float32LE lane across all 3,200 keyframes. A frozen static-recipe plus Undo-gold ledger matches its delta on 2,004/2,100 D7 and 941/1,000 H3 transitions, including 16/16 sale-Undo and 35/36 purchase-Undo intervals. `trunc(totalGold - spentLike)` matches only 967/2,170 D7 and 354/1,030 H3 current-gold labels; adding cumulative static sale proceeds improves this to 1,459/3,200 combined but does not close the residual. A full contiguous/stride-2 decoded `0x02EB` integer/Float32 scan finds no residual correction field; its best candidate is a constant zero. A corrected replay-aware multiplicity gate also freezes all eight exact one-per-champion/keyframe families across the 3,200 snapshots; outside the separately audited `0x02EB`, no raw or profile-permuted UInt/Float32 scalar with stride 1..8 reproduces current gold in D7 and H3. Spendable gold remains unpromoted
- every one of the 3,200 exact-build champion/keyframe `0x0081` payloads has six trailing `(0xC0|0xC3) · ?? · ?? · 0xE6` records and reuses the established item-ID symbol grammar, but this is not current inventory: an offline Timeline reducer finds false-extra decoded items in 675/2,170 D7 and 374/1,030 H3 snapshots. Strict record-activity labels cover 8,137 D7 and 4,014 H3 records, but no one/two-bit affine or contiguous one-to-eight-bit record-local rule is conflict-free. The stronger whole-record collision gate finds 49 D7 and 28 H3 byte-identical, same-ordinal records with opposite activity labels, proving activity requires component-level or temporal context. Per-ordinal component-prefix/sibling-context searches also find zero conflict-free D7 contiguous one-to-eight-bit lookups and zero one/two-bit affine rules for every ordinal, rejecting a simple surrounding activity mask before H3 is consulted. A research-only `0x03F9` removal-slot candidate covers all 2,074 profiled removals and all seven values; 72/72 strict trinket replacements validate candidate slot 6, while the reverse historical record ordinal agrees in 108/111 uniquely labelled sales. Another 181 isolated add/record-change anchors cover all six main candidate slots, but no contiguous one-to-eight-bit add-payload lookup is conflict-free on D7. A bounded same-time scan across every packet family finds only the variable-length `0x0369` add family in all 126 D7 anchors and no companion-field lookup. Separately, 21 no-item-event same-multiset record reorderings include five with no profiled item operation and repeated `0x0146` payloads with contradictory record moves, so they are not physical swap labels. Expanded forward research adds length-11 trinket changes plus rare length-16/17 records: the deterministic replay-only candidate reaches 9/70 D7 and 5/30 H3 exact final slot tracks, while symbolic placement reaches 23/70 and 13/30 final anchors; a static initial-3340 seed improves scores but is explicitly non-promotable. The 31 exact chunk-`0x0081` Undo anchors serialize post-Undo item records; a replay-only fresh-sale linkage identifies nine sale-Undo restored items at 6/6 D7 plus 3/3 H3, zero wrong, while purchase Undo remains unresolved. The sale-Undo-aware backward reducer reverses 4,280/6,945 groups, reaches the beginning for 50/100 tracks, encounters 72/116 sales, and uniquely links five sale identities with 5/5 exact offline validation, but completed tracks retain hundreds or thousands of possible histories. A stronger offline Timeline-oracle slot trace reverses 366/3,230 label groups but no participant reaches the beginning; the exact identity balance across 4,977 Timeline labels closes for 0/100 participants, leaves 858 positive and 13 negative units, and implies more than seven starting units on 36 tracks. The two Viego tracks alone account for 198 positive units (131 and 67), strongly implicating possession-copy item events; non-Viego system/perk, consumable, trinket, and support transformations also remain unmatched. Timeline is therefore useful only as an offline semantic oracle, not a complete inventory reducer. Sold-item identity, slots 0-5, instances, counts, add placement, and current state remain unpromoted
- there is no valid replay-native movement or position decoder; the provisional `0x00DC` raw-coordinate interpretation was disproven and removed from the runtime, and `0x01AB` has no promoted coordinate/waypoint grammar. A maintained exact-build patch-16.14 gate proves 6,314/6,314 consecutive `0x0328`→`0x0170` companion pairs plus 1,494 unpaired `0x0328` blocks, and 759 pair `blockParam` values exactly equal replay-decoded ward entity IDs after placement (all 29 links with known conservative removal precede it), but this bounded generic entity-handle relation exposes no operation, owner/champion, payload, waypoint, or coordinate grammar. A separate D7/H3 gate over all 3,200 champion snapshots rejects raw/profile-permuted signed Int16 and Float32 direct position lanes in substantial `0x0081`, `0x02EB`, and `0x047A` families; best affine RMSE remains about 3,763 X and 3,622 Y map units
- building and turret-plate signatures are correlations with false positives, not runtime decoders
- external profile revision `2026-07-26-cross-patch-xp`, SHA-256 `813838149a17a955d6fe07a3cc6e85855595b9e519d9e75f8ded935fe466d523`, fingerprint `fnv1a64:c52aa67610f4495c`; its exact-build stat profiles validate 16,760/16,760 replay-native XP/level projections and lane/jungle-CS snapshots under the frozen exact-or-ordering-boundary rules across all 57 replays. For build `16.14.794.5912` it also validates 684/684 kills, 99/99 objectives, 1,477/1,477 ward placements, 484/484 ward removals, 1,300/1,300 final player-stat values, 193/193 purchase-linked resulting-item updates, 1,278/1,278 direct add-only item purchases, and 116/116 item-sale operations; ward position has zero valid candidates and remains unavailable
...
1. preserve the promoted cross-patch XP/level and lane/jungle-CS gates; do not treat nullable historical total gold as decoded
2. decode current gold and health/resource state from component/bundle grammar; the direct 16.14 champion-snapshot Float32 scan is negative, and Timeline values must never be runtime inputs
...
5. treat `docs/chat.md` only as historical product context, not current decoder truth
6. inspect the actual repo state before assuming planned work is still unfinished
7. prefer continuing from the current Wasm-backed frontend rather than rebuilding scaffolding
