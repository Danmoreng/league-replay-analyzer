...
- the default web landing page is a focused timeline product view: the scrubber and event list show only kills and objectives, while compact champion cards show portrait, player, role, scrub-time K/D/A, and available derived level/lane CS. Ward, purchase, and sale streams remain Research & Debug concerns and do not appear on the product timeline. Cumulative total gold is not relabelled as current spendable gold, and partial item-operation streams are not presented as inventory slots or current inventory. Final inventories, fake health/resource bars, a minimap, decoder explanations, and ward-position research controls are absent. Current spendable gold and complete timeline inventory remain unavailable until replay-native decoders pass promotion. Static Data Dragon champion icons use the exact replay-build-to-Data-Dragon-version pin (`16.14.794.5912` to `16.14.1`), never a runtime `latest` lookup, and never create replay state
...
- keyframe outer framing and champion ownership are exact. For exact build `16.14.794.5912`, every champion/keyframe has an exact 366-record `0x01EB` directory connected to one 1,479-byte `0x02EB` block by the exact bridge `0x0233 · (0x0306 0x0306)^k · 0x0452 · 0x007D?`, `k=0..4`; the `0x01EB` tokens remain a non-semantic physical directory. Separately, a D7-frozen profile-pinned 256-byte cipher→plain permutation (SHA-256 `c9be1f4971505dcc7c4366329366794108c1b031060039a2bcfd2d60134ed4be`) produces replay-native interleaved Float32LE keyframe snapshots: XP `[83,85,87,89]`, total gold `[115,117,119,121]`, integral lane CS `[123,125,127,129]`, and neutral CS `[131,133,135,137]`; level is derived from XP and replay-embedded final cap, while neutral CS uses the D7-frozen `floor(value + 1e-5)` projection. On 2,170 D7 and 1,030 H3 snapshots neutral CS validates 3,200/3,200 exactly. This profile-backed surface is implemented through C++, Native, Wasm, and the product UI and fails closed outside the exact external profile. Current gold, health/resources, full inventory, movement, and the remaining inner state grammar remain unresolved
...
- external profile revision `2026-07-24`, SHA-256 `ca5696864d60d9a7667cfbe3221be1303d3f248de10983b05389fc8275eeaf7a`, fingerprint `fnv1a64:5d6e6dfe099ce86f`, for build `16.14.794.5912` validates 684/684 kills, 99/99 objectives, 1,477/1,477 ward placements, 484/484 ward removals, 1,300/1,300 final player-stat values, 193/193 purchase-linked resulting-item updates, 1,278/1,278 direct add-only item purchases, 116/116 item-sale operations, 3,200/3,200 keyframe levels, 3,198/3,200 floored XP values (the two differences are frozen ordering boundaries), and 3,200/3,200 projected neutral-CS values across the ten saved replay/API fixtures; ward position has zero valid candidates and remains unavailable
