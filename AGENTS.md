...
- patch-16.9 inventory research decodes add/update item IDs and a sale-operation class; for exact build `16.14.794.5912`, the productive external-profile-only `rofl-replay-purchase-linked-item-updates/v1` stream emits the strict replay-only subset of 193/193 offline-validated purchase-linked resulting-item updates (D7 130, H3 63), with zero extras/wrong IDs and at most 1 ms timing delta; 2,326/2,519 profiled add/update packets remain unavailable, and the stream provides no general purchase classification, removed component/item identity, slot, instance, count/charges, removal, full inventory, price/gold, or undo state. Two unseen item-ID input symbols still fail closed, and an all-segment scan of 3,266 `0x0369` blocks finds neither symbol. Direct Recipe-/Removal-Ordinal hypotheses are intentionally not promoted: they cover only 113/193 exactly, while the Arity gate reaches 139 but has 26 false positives
...
- external profile build `16.14.794.5912` validates 684/684 kills, 99/99
  ...
  1,300/1,300 final player-stat values, plus the strict 193/193
  purchase-linked resulting-item-update subset, across the ten saved replay/API
