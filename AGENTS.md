...
- keyframe outer framing and champion ownership are exact. For patch 16.14, every champion/keyframe has an exact 366-record `0x01EB` directory connected to one 1,479-byte `0x02EB` block by the exact bridge `0x0233 · (0x0306 0x0306)^k · 0x0452 · 0x007D?`, `k=0..4`; `0x01EB` `payload[3..]` yields 366 unique, stable tokens across all 2,170 D7 and 1,030 H3 owner-segments. The wider group has an exact partial typed-stream skeleton: all 63,905 D7 and 32,567 H3 `0x0306` payloads are a two-/three-byte base plus zero to two four-byte extensions, four contiguous `0x027C` records follow `0x02EB`, and exactly four contiguous `0x011A` records occur between `0x02EB` and the unique `0x0151` end. This proves serialization boundaries only: no index→cell/value semantics or state field is decoded. Direct, descriptor-guided, and raw typed-stream Gold/CS relations are negative. The existing exact but non-semantic `totalGold` and lane-CS change correlations across 3,100 transitions remain post-hoc/non-unique; neither exposes a numeric value or field boundary, and a bounded 1,126-candidate stride-scalar audit plus discrete champion-owner CS event searches fail. No numeric gold decoder exists. `0x0217` and generic NPC-lifecycle searches also yield no replay-native CS/minion/monster credit decoder; schema v4 freezes two exact D7→H3 mirror ranges and falsifies simple two-byte cell/mask layouts, while one identical local tuple maps to both lane CS 91 and 92, so the inner stateful replication/component record grammar remains unresolved
...
- external profile revision `2026-07-21`, SHA-256 `f690f77926c09d28998c52e5a56044c4575a23a268f49328c071be02d2359cce`, fingerprint `fnv1a64:10b2b8d2727009a0`, for build `16.14.794.5912` validates 684/684 kills, 99/99 objectives, 1,477/1,477 ward placements, 484/484 ward removals, 1,300/1,300 final player-stat values, 193/193 purchase-linked resulting-item updates, 1,278/1,278 direct add-only item purchases, and 116/116 item-sale operations across the ten saved replay/API fixtures; ward position has zero valid candidates and remains unavailable
...
2. decode the variable-prefix and typed-stream grammar behind the exact patch-16.14 366-record `0x01EB` directory, using its parallel `0x02EB` block plus the exact but non-semantic `totalGold` and lane-CS change correlations only as search constraints; do not treat directory indices/tokens as decoded state before reconstructing numeric gold, CS, XP, level, health, and resources
