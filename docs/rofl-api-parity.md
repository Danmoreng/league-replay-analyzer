...
The full checkpoint also regenerates and verifies the strict replay-only position diagnostic before export, asserting that identity priors and validation-derived support hypotheses are disabled, the focused replay assigns only 4/10 participants, and 3/4 assigned tracks pass offline validation.

There is now a stricter position diagnostic that also disables validation-derived movement `supportHypotheses`:

```powershell
npm run assign:movement -- --artifact-dir artifacts/EUW1-7840220945 --priors-path artifacts/EUW1-7840220945/no-priors.json --output-path artifacts/EUW1-7840220945/participant-movement-strict-replay-only-probe.json --ignore-support-hypotheses
npm run validate:assigned-movement -- --participant-movement-path artifacts/EUW1-7840220945/participant-movement-strict-replay-only-probe.json --output-path artifacts/EUW1-7840220945/assigned-movement-strict-replay-only-probe-validation-report.json
```

On `EUW1-7840220945`, the strict replay-only diagnostic assigns 4/10 participants and 3/4 pass offline validation. Across the current 20 replay patch `16.9` corpus, the same strict mode assigns 38/200 participant tracks and 20/38 pass offline validation. This is the current runtime-promotion baseline for movement identity; the looser no-priors diagnostic remains useful evidence, but still uses validation-derived support hypotheses and must not be treated as runtime-promotable position data.

A strict threshold probe also runs without identity priors or validation-derived support hypotheses:

```powershell
npm run assign:movement -- --artifact-dir artifacts/EUW1-7840220945 --priors-path artifacts/EUW1-7840220945/no-priors.json --output-path artifacts/EUW1-7840220945/participant-movement-strict-min0.44-probe.json --ignore-support-hypotheses --min-assignment-score 0.44
npm run validate:assigned-movement -- --participant-movement-path artifacts/EUW1-7840220945/participant-movement-strict-min0.44-probe.json --output-path artifacts/EUW1-7840220945/assigned-movement-strict-min0.44-probe-validation-report.json
```

On the focus replay, strict `min-assignment-score=0.44` assigns 7/10 participants and 4/7 pass offline validation. Across the 20 replay patch `16.9` corpus, it assigns 73/200 participant tracks and 33/73 pass offline validation. This confirms that lower strict thresholds recover candidate coverage, but the added candidates are not reliable enough for runtime promotion.

Summarize the available movement diagnostics with:

```powershell
npm run run:movement-diagnostics -- --version-group 16.9 --skip-default --summary-output-path artifacts-keyframes/movement-diagnostics-summary-16.9.json
npm run run:movement-diagnostics -- --version-group 16.9 --skip-default --skip-no-priors --skip-min-score-probe --skip-strict-replay-only --summary-output-path artifacts-keyframes/movement-diagnostics-summary-16.9.json
...
npm run summarize:movement-oracle -- --version-group 16.9
npm run verify:movement-oracle -- --version-group 16.9
```

Current `movement-diagnostics-summary-16.9.json` scorecard:

- default prior-assisted diagnostics: `105 / 200` assigned, `85 / 105` offline-passing, identity priors present on `20 / 20` replays
- no-priors diagnostics: `99 / 200` assigned, `83 / 99` offline-passing, validation-derived support hypotheses present on `20 / 20` replays
- no-priors `min-assignment-score=0.46` probe: `107 / 200` assigned, `84 / 107` offline-passing, validation-derived support hypotheses present on `20 / 20` replays, so lower threshold mostly adds noise
- strict replay-only diagnostics: `38 / 200` assigned, `20 / 38` offline-passing, `0` complete replays
- strict replay-only `min-assignment-score=0.44` probe: `73 / 200` assigned, `33 / 73` offline-passing, `0` complete replays, so threshold relaxation alone is not a promotion path
- strict replay-only `min-assignment-score=0.44` duplicate-entity probe: `144 / 200` assigned, `46 / 144` offline-passing, `6` complete replays, `0` perfect replays, so reusing the same entity for multiple participants increases coverage but mostly exposes collision ambiguity rather than solving identity
- strict replay-only `min-assignment-score=0.44` max50 extraction probe: `113 / 200` assigned, `40 / 113` offline-passing, `3` complete replays, `0` perfect replays, so wider extraction improves assignment coverage but lowers precision and still cannot be emitted as runtime position data
- strict replay-only `min-assignment-score=0.44` max50 supplemented extraction probe: `136 / 200` assigned, `41 / 136` offline-passing, `5` complete replays, `0` perfect replays, so supplementing sparse promoted pattern sets improves coverage but mostly adds ambiguous/wrong owners
- strict replay-only `min-assignment-score=0.44` current max128 extraction probe: `155 / 200` assigned, `59 / 155` offline-passing, `8` complete replays, `0` perfect replays
- strict replay-only current max128 with the reduced role-anchor score profile: `165 / 200` assigned, `60 / 165` offline-passing, `12` complete replays, `0` perfect replays
- strict replay-only current max128 with reduced role-anchor scoring and alias preservation: `190 / 200` assigned, `74 / 190` offline-passing, `19` complete replays, `0` perfect replays, but it produces duplicate claimed entity keys and remains diagnostic-only

The strict artifacts now include `assignmentConfidence` for each assignment, with score margins against the best competing entity for the same participant and the best competing participant for the same entity. On the current corpus summary, strict replay-only passing assignments have a higher average entity margin than failing assignments (`0.068` vs `0.024` at the default strict threshold, `0.044` vs `0.011` at strict `min-assignment-score=0.44`), but participant margins overlap heavily (`0.012` vs `0.016` at the default strict threshold). This makes entity ambiguity a useful diagnostic, but not yet a standalone participant identity gate.

The movement summary also records diagnostic-only quality gates. `teamScore >= 0.60 && entityScoreMargin >= 0.02` improves strict default precision to `8 / 10` and strict `min-assignment-score=0.44` precision to `8 / 11`, but the coverage is far below the required `200 / 200` participant tracks. `teamScore >= 0.65` keeps `13` strict `min-assignment-score=0.44` assignments with `10` passing. These gates are useful for ranking candidates during decoder research, but they do not replace a stable replay-native participant identity anchor.

The same summary now splits unmatched participants by the top rejected candidate status. In strict replay-only mode, unmatched participants have top candidates for every role, but those candidates are split between below-threshold and already-assigned collisions. At strict `min-assignment-score=0.44`, the threshold blocker nearly disappears and collision dominates: BOTTOM `31 / 31`, JUNGLE `24 / 25`, MIDDLE `20 / 21`, TOP `25 / 27`, and UTILITY `22 / 23` unmatched participants have a top candidate already assigned to another participant. This means the immediate blocker is not absent movement extraction; it is unstable entity-to-participant disambiguation.

The summary also records collision-pressure buckets for assigned entities that are top rejected candidates for unmatched participants. In strict replay-only mode, `18` assigned entities have `7+` competing rejected participants (`11` passing, `7` failing). At strict `min-assignment-score=0.44`, `16` assigned entities have `7+` competitors (`9` passing, `7` failing). High collision pressure therefore marks ambiguous shared candidates, not correctness by itself; the decoder still needs an independent replay-native identity signal to decide which participant owns each high-pressure entity.

Each assignment also records whether the selected participant is the top-scoring owner for that entity. In strict replay-only mode, top-owner assignments are `17` passing / `12` failing, while non-top owner assignments are `3` passing / `6` failing. In strict `min-assignment-score=0.44`, top-owner assignments are `23` passing / `25` failing, while non-top owner assignments are `10` passing / `15` failing. Non-top ownership is a useful warning sign, but top-owner choices still fail often enough that owner rank is not a promotion gate on its own.

The `assignmentRiskMatrix` combines team geography, entity margin, owner rank, and collision pressure. The best current pocket is `teamStrong|entityMarginStrong|topOwner`: strict replay-only has `5 / 5` passing assignments in the `collision7Plus` bucket and strict `min-assignment-score=0.44` has `7 / 10` passing across its `collision1To3`, `collision4To6`, and `collision7Plus` buckets. That is useful for ranking likely-correct candidates, but it covers too few assignments to emit runtime `participantFrames.position`.

A solver probe can bias assignment toward the top-scoring replay-only owner for each entity (`--prefer-top-entity-owner`). On the focus replay this is neutral at the default strict threshold (`4 / 10` assigned, `3 / 4` passing), but harmful at strict `min-assignment-score=0.44` (`7 / 10` assigned, `2 / 7` passing versus `4 / 7` without the bias). Across the 20 replay patch `16.9` corpus, top-owner bias is also neutral at strict default (`38 / 200` assigned, `20 / 38` passing) and worse at strict `min-assignment-score=0.44` (`73 / 200` assigned, `30 / 73` passing versus `33 / 73` without the bias). Owner-rank bias should stay diagnostic-only until a stronger identity signal explains when the top-owner score is trustworthy.

Another solver probe allows duplicate entity ownership (`--allow-duplicate-entities`) to test whether the one-to-one assignment constraint is the main blocker. It assigns `144 / 200` strict replay-only tracks at `min-assignment-score=0.44`, but only `46 / 144` pass offline validation. The focus replay illustrates the failure mode: `49607-0xF1-h7|12|0|3|normal` is claimed by TOP, JUNGLE, and MIDDLE, but only the MIDDLE claim passes. This keeps duplicate-entity assignment diagnostic-only; runtime position emission still needs a replay-native participant identity signal that chooses the correct owner for shared high-scoring trajectories.

An offline oracle summary, `movement-oracle-coverage-summary-16.9.json`, measures the upper bound using validation-derived `supportHypotheses`. It is marked `runtimeInput=false` and `runtimeApiData=false` and must not be used for runtime identity. The current oracle finds candidate hypotheses for `185 / 200` participants, but only `80 / 200` participants have a validation-passing movement hypothesis and only `1 / 20` replays reaches all 10 participants. TOP is the weakest role even with the oracle (`8 / 40` passing), while BOTTOM has `17 / 40`, JUNGLE `18 / 40`, MIDDLE `19 / 40`, and UTILITY `18 / 40`. This means the remaining movement gap is not only assignment policy; the extracted candidate set itself is still incomplete for full-corpus position promotion.

The same oracle summary now records why non-passing participants fail. The dominant blocker is coordinate quality, not total absence: `103` failing participants have `rmse_above_0.18`, `55` have `axis_below_0.55`, and only `15` have `no_candidate`. TOP carries the most high-RMSE failures (`28`). The top failing candidate families are `50944-0x19-h0` and `50944-0xC1-h0` (`24` each), followed by missing candidates (`15`), `61721-0x00-h9` (`13`), `6599-0xF1-h7` (`12`), `61889-0x00-h1` (`10`), and `49607-0xF1-h7` (`9`). The next extraction target should therefore be better coordinate calibration/row selection for those families, especially TOP, before further assignment-policy tuning.

`movement-candidate-failure-sources-16.9.json` looks one layer earlier at `movement-candidate-matches.json`, preserving coordinate-model source/support metadata. That diagnostic reaches `105 / 200` passing participants before extraction/provisional-schema filtering, so some usable candidates are lost before `extracted-movement.json`. The remaining `95` candidate-match failures are split between unsupported transforms (`coordinateModelSource=none`, `46`) and signature-backed transforms (`49`), and high RMSE dominates both (`42` unsupported, `49` signature-backed). The largest failing families at the candidate-match layer are `50944-0xC1-h0` (`21`), `6599-0xF1-h7` (`20`), `50944-0x19-h0` (`15`), and `61721-0x00-h9` (`15`). This points to two concrete decoder tasks: preserve more of the already-passing candidate-match rows through extraction, and improve coordinate-model calibration for the signature-backed high-RMSE families.

`movement-candidate-to-extracted-oracle-comparison-16.9.json` compares any validation-passing candidate-match row against the extracted oracle participant rows. It finds `78` preserved passing participants, `64` participants with a passing candidate-match row lost before extraction, `2` extracted-only passes, and `56` participants with no pass at either layer. The lost passing rows are concentrated in `50944-0x19-h0` (`15`), `6599-0xF1-h7` (`13`), `61721-0x00-h9` (`12`), and `50944-0xC1-h0` (`9`), split between signature-backed (`33`) and unsupported (`31`) coordinate models. The loss reasons are now explicit: `28` are absent from the provisional schema, `20` are schema rank `40+`, `6` are rank `20-39`, `1` is rank `10-19`, `5` are rank `0-9` but still not selected by the fallback extraction policy, `3` have the extracted candidate present but lose the passing hypothesis, and `1` selects a different extracted candidate. The immediate extraction fixes are therefore: first revise extraction selection for schema-present rows instead of only raising `--max-patterns`, then improve provisional schema promotion for absent passing rows.

A diagnostic extraction-width probe (`movement-extraction-probe-16.9-max50.json`) reruns extraction with `--max-patterns 50` into `extracted-movement-max50.json`. Its oracle summary improves from `80 / 200` to `103 / 200` validation-passing participants and reduces high-RMSE failures from `103` to `80`, confirming that extraction selection is dropping useful schema-present candidates. It still has only `1 / 20` complete replays and TOP remains weak (`13 / 40`), so this is not a promotion path by itself. It does show the next implementation should make extraction selection smarter and candidate-aware rather than simply preserving the current narrow fallback list.

The same max50 movement file was also passed through strict replay-only assignment with identity priors and support hypotheses disabled. That raises assignment coverage to `113 / 200` participants and reaches `3 / 20` complete replays, but only `40 / 113` assigned tracks pass offline validation and `0` replays are perfect. This confirms the current bottleneck is two-sided: extraction needs to keep more good candidates, and assignment still needs a replay-native participant identity signal before `participantFrames.position` can become runtime output.

The extractor now supplements promoted movement patterns with ranked fallback patterns when the promoted set is smaller than the requested extraction width. The `extracted-movement-max50-supplemented.json` probe raises oracle coverage to `119 / 200` validation-passing participants and `2 / 20` complete oracle replays, and reduces lost candidate-match passes from `42` to `29`. The strict replay-only assignment probe over the same supplemented movement file assigns `136 / 200` participants and reaches `5 / 20` complete assignment replays, but only `41 / 136` assignments pass offline validation and `0` replays are perfect. This is an extraction improvement, not a runtime promotion: the extra candidates still need a replay-native identity anchor to choose the correct participant owner.

The one-to-one movement assignment solver now uses participant-mask dynamic programming rather than JavaScript 32-bit entity masks. This keeps the one-entity-per-participant constraint exact for wide extraction probes with more than 31 candidate entities. The corrected solver preserves the current scorecard above, so the remaining gap is not an assignment-mask artifact; it is still insufficient replay-native identity evidence.

`movement-assignment-oracle-gap-16.9.json` joins the strict replay-only max50 supplemented assignments with the offline movement oracle. It shows `119 / 200` participants have a validation-passing oracle candidate in the supplemented movement file, but strict replay-only assignment gets only `41` passing assignments. The main identity blockers are now explicit: `59` participants have a passing oracle candidate available but are assigned the wrong entity, and `25` have a passing oracle candidate but remain unassigned. Role pressure is worst for UTILITY (`15` wrong-entity assignments), followed by JUNGLE (`13`), MIDDLE (`12`), TOP (`11`), and BOTTOM (`8`). This confirms the next decoder target is participant/entity linkage, not more assignment threshold tuning.

The wrong-entity breakdown is mostly cross-family rather than nearby same-row ambiguity: `45 / 59` wrong assignments pick a different family than the oracle entity, `10 / 59` pick the same family but a different slot, and only `4 / 59` pick the same family+slot group. The largest assigned/oracle family confusions are `50944-0x19-h0 -> 6599-0xF1-h7` (`5`), same-family `6599-0xF1-h7` (`5`), and `49607-0xF1-h7 -> 61889-0x00-h1` (`4`). That points away from local row-neighbor scoring tweaks and toward a replay-native participant/entity linkage signal that can reject whole wrong families for a participant.

The same gap summary records where the offline-correct oracle entity lands in the replay-only ranking. After expanding the assignment debug window to the top 64 alternatives, wrong-entity assignments still have the oracle entity outside the recorded alternatives in `34 / 59` cases, rank `2-3` in `10`, rank `4-5` in `6`, rank `6-10` in `7`, and rank `11-32` in only `2`. For unassigned participants with a passing oracle candidate, the oracle entity is top-ranked in `5 / 25`, rank `2-3` in `8`, rank `6-10` in `2`, and outside the recorded alternatives in `10`. This means simple tie-breaking can recover only a minority of cases; most wrong assignments need a new replay-native feature that materially changes family/entity ranking.

The score-gap buckets lead to the same conclusion. For wrong-entity assignments, the oracle replay-only score is unknown because the oracle entity is outside the recorded alternatives in `34` cases; among the recorded cases, only `9` are within `0.01` of the selected wrong entity and `1` more is within `0.03`. The remaining recorded oracle candidates are farther away: `3` at `0.03-0.06`, `5` at `0.06-0.10`, and `7` above `0.10`. Solver tie-breaking can help a small near-tie slice, but the dominant failure is poor replay-only ranking of the correct movement family/entity.

Scalar stat row proximity is not that feature. In the `59` wrong-entity assignments, neither the assigned nor oracle movement family has a decoded scalar slot for the participant in `52` cases; the wrong assigned family is closer in `6`, and the oracle family is closer in only `1`. The existing scalar-neighborhood score should remain a weak hint, not a primary identity bridge for movement.

The replay-only score components also do not isolate wrong assignments cleanly. In the max50 supplemented gap summary, passing assignments average `teamScore=0.459`, `roleScore=0.681`, `entityQuality=0.950`, while wrong-entity assignments average `teamScore=0.471`, `roleScore=0.613`, `entityQuality=0.961`. Role geography is somewhat better for correct assignments, but team geography and entity quality are not; a simple geography/entity-quality reweighting is unlikely to fix the cross-family identity problem.

The reduced role-anchor score profile is the current best strict one-to-one replay-only assignment candidate, but it is still not promotable. Its current max128 gap summary assigns `165 / 200` participants, validates only `60 / 165`, and still has `61` wrong-entity assignments. `movement-score-component-gaps-16.9-current-max128-reduced-role.json` shows only `24` of those wrong assignments have the offline-correct oracle entity visible in the participant alternatives; `37` remain hidden. Expanding diagnostic alternatives to 512 did not reveal additional oracles, so the issue is not the recorded top-64 window.

`movement-hidden-oracle-sources-16.9.json` classifies those hidden validation-passing oracle candidates. The key result is that all `73` hidden oracle cases are alias/canonicalization cases: `61` where the kept alias entity was assigned elsewhere and `12` where the kept alias entity was unassigned. The alias-preserving diagnostic confirms that canonicalization is a real blocker by raising strict replay-only coverage to `190 / 200` and passing tracks to `74`, but it also introduces `9` duplicate claimed entity keys. The next implementation target is therefore alias-aware entity grouping that preserves useful coordinate aliases without allowing duplicate or ambiguous owner claims.

`movement-position-goal-audit-16.9.json` is the movement-specific completion gate for promoting `participantFrames.position`. It maps the goal directly to strict replay-only diagnostics and currently reports `not_complete`, with `2 / 9` checks satisfied. The satisfied checks are diagnostic availability and no identity-prior/support-hypothesis dependence. The open checks are full corpus assignment coverage (`165 / 200` on the current reduced-role promotion candidate), validation-passing tracks (`60 / 200` required target coverage), perfect replay coverage (`0 / 20`), closing the assignment/oracle identity gap (`61` wrong-entity assignments plus hidden alias/canonicalization cases), selecting a replay-native entity identity signal, separating oracle entities from wrong assignments with replay-only scoring, and preserving all discovered validation-passing movement candidates through extraction. Run `npm run audit:movement-position-goal -- --require-complete` as the negative gate; it must fail until movement promotion is actually achieved.

The movement position goal audit is also mirrored into `rejectedCandidateArtifacts.positions.movementPositionGoalAudit`, `fieldCoverage.positions.movementPositionGoalAudit`, and `roflDerivedFieldMap.timeline["info.frames[].participantFrames[].position"].movementPositionGoalAudit` in the API-shaped artifact. This keeps runtime consumers aligned with the same not-complete movement promotion gate used by the checkpoint.

Strict replay-only role/family blockers from that summary:

- TOP remains the weakest labelled role: `1` passing assignment, `6` failing assignments, and `33` unmatched participants
- UTILITY is also weak: `3` passing assignments, `6` failing assignments, and `31` unmatched participants
- the noisiest strict assigned families are `61889-0x00-h1` (`2` passing / `5` failing), `50944-0x19-h0` (`4` passing / `5` failing), and `40606-0x31-h14` (`0` passing / `4` failing)
- the most common top rejected families for unmatched participants are `50944-0xC1-h0`, `50944-0x19-h0`, `61889-0x00-h1`, `49607-0xF1-h7`, and `6599-0xF1-h7`
- slot-level strict evidence is still too sparse for promotion: the largest clean passing slots are only `2` passes each (`49607-0xF1-h7|slot=14` and `50944-0x19-h0|slot=17`), while the failing slots are mostly one-off spread across the same recurring families
- top rejected family/slot evidence is broad rather than row-stable; common rejected slots include `50944-0xC1-h0|slot=16`, `50944-0x19-h0|slot=23`, `61889-0x00-h1|slot=12`, `61889-0x00-h1|slot=16`, `6599-0xF1-h7|slot=17`, and `49607-0xF1-h7|slot=14`

Next decoder target for movement should therefore be alias-aware replay-native row/entity identity for those recurring families, especially TOP/UTILITY ambiguity, not assignment threshold tuning or wider alternative logging.
...
- `rejectedCandidateArtifacts.reconstructionRowIdentity.blockerMatrix` separates the current 241-family identity blockers: `241-0x02` is blocked primarily by duplicate row bytes, while `241-0x04` is blocked by low row continuity with all 10 rows still `unstable_identity`
...
- `keyframe-handle-graph-candidate-scores.json` now includes a `nearMissSummary`. The top handle-graph candidate is close to the `investigate` score threshold (`0.333 / 0.35`) but has only `1 / 2` assigned replay support for investigation and `1 / 4` for strong promotion. The broadest high-support candidates reach `11` assigned replays, but are diffuse across `30+` assigned rows with high entropy, so the current handle graph evidence needs row-specificity improvement rather than broader coverage alone.
- The handle-graph `nearMissSummary` is mirrored into `rejectedCandidateArtifacts.nonFinalScalarIdentity.handleGraphRowLinkCandidates` and `identityLinkage.nonFinalScalarIdentity.handleGraphRowLinkCandidates`, so runtime metrics consumers and checkpoint verifiers see why the identity bridge remains blocked without reading the raw scorecard.
- Verify the handle-graph scorecard directly with `npm run verify:keyframe-handle-graph`; the full parity checkpoint runs this verifier before exporting the ROFL API metrics artifact.
